...
class FavoritesHistoryAdapter(
    private val list: MutableList<QrHistoryItem>,
    private val onItemClick: (QrHistoryItem) -> Unit
) : RecyclerView.Adapter<FavoritesHistoryAdapter.HistoryViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HistoryViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.favorites_history_item, parent, false)
        return HistoryViewHolder(view)
    }

    override fun getItemCount() =  minOf(list.size, 3)
